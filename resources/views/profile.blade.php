...
<div class="md:w-full lg:w-6/12 2xl:w-5/12 bg-gray-100 border border-gray-200 p-5 container mx-auto rounded md:container">
        ...
        <div>
            ...
            @if ($follow <= 0)
                ...
                <form class="inline" action="{{ route('follow', ['id' => $user->id]) }}" method="POST">
                    @csrf
                    <button class="inline-block rounded text-sm px-3 py-2 text-white bg-blue-600 font-medium">Follow</button>
                </form>
                @endif
            @else
                @if (Auth::user()->id != $user->id)
                <form class="inline" action="{{ route('unfollow', ['id' => $user->id]) }}" method="POST">
                    @csrf
                    <button class="inline-block rounded text-sm px-3 py-2 text-white bg-blue-800 font-medium">Unfollow</button>
                </form>
                @endif
            @endif
        </div>
    ...
    @if (count($blogs) <= 0)
        <p class="mt-5 text-sm mx-1 rounded bg-blue-700 p-3 text-white font-medium">User didn't post any blogs.</p>
    @endif
    @foreach ($blogs as $blog)
    <div class="bg-white p-3 my-2 border border-gray-200 rounded">
        <div class="flex justify-between mb-3">
            <h3 class="inline text-md bg-blue-700 px-2 rounded text-white">{{ $blog->title }}</h3>
            @if (Auth::user()->id == $user->id)
